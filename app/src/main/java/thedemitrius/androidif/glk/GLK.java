...
public class GLK implements GLKConstants
{
	...
	public GLK(final Activity context,final GLKFrame frame,String homeDir) throws IOException
	{
		...
		files.put(1, FileRef.createFileRef(context,homeDir,"state",5));
		nextFileID=2;
	...
	public int windowOpen(int split,final int method,int size,int wintype,int rock) throws CloneNotSupportedException, InvocationTargetException
	{
		if (windows.isEmpty()){
			...
			final Window oldWin= windows.get(split);
	...
	private Window windowCreate(int method,int size,int wintype,int rock,boolean key) throws CloneNotSupportedException
	{
		...
		case WINTYPE_TEXTBUFFER:
			...
			streams.put(nextStreamID, newStream);
			nextStreamID++;
	...
	{
		final Window winObj= windows.get(win);
	...
	{
		streams.put(nextStreamID, new MemoryStream(nextStreamID,buf,bufStart,bufLen,mode,rock,false));
		nextStreamID++;
	...
	{
		streams.put(nextStreamID, new MemoryStream(nextStreamID,buf,bufStart,bufLen,mode,rock,true));
		nextStreamID++;
		return (nextStreamID-1);
	}
	...
	{
		FileRef fileObj= files.get(file);
		if (fileObj==null)
			return (0);
		
		streams.put(nextStreamID, new FileStream(nextStreamID,fileObj,mode,rock,false));
		nextStreamID++;
	...
	public String copySaveDir(String dirName,FileRef[] undoFiles,int mode) throws IOException, ClassNotFoundException, InterruptedException
	{
		...
		if (mode==FILEMODE_WRITE){
			...
			dir.mkdir();
			for (FileRef undoFile : undoFiles) {
				if (undoFile.exists()) {
					File output = new File(dir, undoFile.getName());
					copyFile(undoFile, output);
				}
			}
		}
		else{
			...
			for (FileRef undoFile : undoFiles) {
				undoFile.delete();
				File input = new File(dir, undoFile.getName());
				if (input.exists())
					copyFile(input, undoFile);
			}
