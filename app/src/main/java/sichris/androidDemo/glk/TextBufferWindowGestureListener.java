...
public class TextBufferWindowGestureListener implements GLKConstants
{
	...
	private boolean onSingleTapConfirmed(MotionEvent e)
	{
		...
		if (hyperlinksActive&&!hyperlinks.isEmpty()){
			int lastLink=(hyperlinks.lastKey()).intValue();
			for (int i=0;i<=lastLink;i++){
				if (hyperlinks.containsKey(new Integer(i))){
					Rect rect=hyperlinks.get(new Integer(i));
	...
	{
		if (hyperlinks.containsKey(new Integer(id))){
			Rect rect=hyperlinks.get(new Integer(id));
			rect.bottom=loc.bottom;}
		else{ 
			hyperlinks.put(new Integer(id), loc);}
	}
