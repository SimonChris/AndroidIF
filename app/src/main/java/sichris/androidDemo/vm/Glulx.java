...
import sichris.androidDemo.Init;
import sichris.androidDemo.glk.GLK;
import sichris.androidDemo.util.DataTransforms;
import sichris.androidDemo.util.FastByteBuffer;
...
public class Glulx implements OPConstants, GLKFunctions
{
	...
	private static int OBJECT_METACLASS=3;
	private static int ROUTINE_METACLASS=4;
	private static int STRING_METACLASS=5;
	private static int SELF=6;
	private static int NUM_ATTR_BYTES=7;
	private static int CPV_START=8;
	...
	private void init(InputStream story) throws IOException, OutOfMemoryError
	{
		...
		{
			extstart=DataTransforms.getInt(buf, 12);
            ...
            try {
				...
				/* Algorithm thanks to Andrew Plotkin... */
                    ...
                    case STREAMCHAR:
						...
						break;
                    case STREAMNUM:
                        glk.putString(stringToBytes(new Integer(values[0]).toString()), 0);
                        break;
                    case STREAMSTR:
	...
	private class HuffmanTree
	{
		...
		Node readTree(int rootaddress, boolean recurse)
		{
			...
			case NON_LEAF:
				...
				if (recurse)
				{
					n.left = readTree(n.leftaddr, recurse);
					n.right = readTree(n.rightaddr, recurse);
				}
				break;
			case TERM:
				break;
